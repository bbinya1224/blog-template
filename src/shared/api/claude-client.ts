...
/**
 * PDF 스타일 분석용 Claude Sonnet 호출
 */
export const analyzePdfStyleWithClaude = async (
  pdfBase64: string,
  systemPrompt: string,
  userPrompt: string
): Promise<string> => {
  try {
    ...
    // Re-use the error handling logic from callClaude if possible, or just throw wrapped error
    // For simplicity, just wrapping here similar to callClaude
    if (error instanceof Anthropic.APIError) {
        if (error.status === 401) {
          throw new AppError(
            'Claude API 인증 실패: API 키를 확인해주세요.',
