...
export function StepIndicator() {
  ...
  const currentStepIndex = STEPS.findIndex(step => pathname.startsWith(step.link));
  return (
      ...
      {STEPS.map((step, index) => {
        ...
        const content = (
          <>
            ...
            <span
              className={
                status === "current" ? "text-gray-900" : undefined
              }
            >
              {step.label}
            </span>
        ...
        return (
          <li key={step.label} className="flex items-center text-sm">
            <Link href={step.link} className="flex items-center hover:opacity-80">
              {content}
            </Link>
