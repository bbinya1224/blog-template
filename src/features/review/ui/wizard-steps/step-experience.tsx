    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center space-y-3 mb-8">
        <div className="inline-block px-4 py-2 bg-gradient-to-r from-orange-500 to-pink-500 rounded-full mb-2">
          <span className="text-white font-bold text-sm">⭐ 가장 중요한 단계입니다 (필수!)</span>
        </div>
      ...
      <div className="bg-gradient-to-br from-orange-50 to-pink-50 border-2 border-orange-200 rounded-2xl p-6 shadow-sm">
        ...
        <ul className="text-sm text-orange-800 space-y-2">
          <li className="flex items-start gap-2">
            ...
            <span><strong>완벽한 문장이 아니어도 괜찮아요!</strong> (예: &ldquo;파스타 좀 짰음, 근데 맛남&rdquo;)</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="text-orange-500 font-bold mt-0.5">•</span>
            <span><strong>맛, 양, 식감, 분위기, 서비스</strong> 등 떠오르는 대로 자유롭게 적어주세요</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="text-orange-500 font-bold mt-0.5">•</span>
            <span>AI가 여러분의 경험을 <strong>1500자 이상의 생생한 리뷰</strong>로 확장합니다</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="text-orange-500 font-bold mt-0.5">•</span>
            <span className="text-orange-900 font-semibold">최소 30자 이상 작성을 권장드려요! 📝</span>
          </li>
        </ul>
      </div>
      ...
      <div className="flex justify-between items-center text-sm">
        <span className={`font-medium ${draftLength === 0 ? 'text-red-500' : isShort ? 'text-orange-600' : 'text-green-600'}`}>
          {draftLength === 0 ? (
      ...
      {isShort && (
        <div className="bg-orange-50 border-l-4 border-orange-400 p-4 rounded-r-lg animate-in slide-in-from-top-2 duration-300">
          <p className="text-sm text-orange-800 font-medium">
            ...
            <br />
            <span className="text-orange-900 font-semibold">예: 맛은 어땠나요? 분위기는요? 서비스는요?</span>
          </p>
        </div>
      )}
    </div>
