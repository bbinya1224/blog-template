...
import type { PlaceCardMetadata } from '../model/types';
...
export function PlaceCard({ metadata, onConfirm, className }: PlaceCardProps) {
  return (
    <div
      className={cn(
        'overflow-hidden rounded-2xl border-2 border-stone-100 bg-white',
      ...
      <div className='space-y-3 p-5'>
        <div className='flex items-start justify-between gap-3'>
          <div className='min-w-0 flex-1'>
            <h3 className='text-lg/tight font-semibold text-stone-800'>
              {metadata.name}
            </h3>
            {metadata.category && (
        ...
        {metadata.tags && metadata.tags.length > 0 && (
          <div className='flex flex-wrap gap-1.5 pt-1'>
            {metadata.tags.map((tag) => (
              <span
                key={tag}
                className='rounded-full bg-orange-50 px-2.5 py-1 text-xs font-medium text-orange-500'
              >
                #{tag}
              </span>
            ))}
        ...
        {metadata.description && (
          <p className='pt-1 text-sm/relaxed text-stone-500'>
            {metadata.description}
          </p>
        )}
      </div>
